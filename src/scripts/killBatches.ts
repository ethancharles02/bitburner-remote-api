...
export function killBatches(ns: NS, host = "", doKillBatches = false) {
    if (doKillBatches) {
...
export async function main(ns: NS) {
    ...
    let doBatchKill = false;
    if (ns.args.length == 1) {
        doSafeKill = ns.args[0] == "1";
        doBatchKill = ns.args[0] == "2";
    }
    if (doSafeKill) {
